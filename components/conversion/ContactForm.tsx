 ...
 *
 * No hidden detection form is needed for this one. `/kontakt` is statically
 * prerendered, so this markup is in the built HTML for Netlify's build-time
 * scanner to find — unlike the jaw form, which only appears on an on-demand
 * route and therefore needs the stand-in in `app/layout.tsx`.
 *
  ...
  return (
    ...
    >
      ...
      <button className={styles.submit} disabled={submitting} type="submit">
        {submitting ? "Odosielame…" : entryExam.cta}
      </button>
    </form>
  );
}
